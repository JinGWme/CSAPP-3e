...
/* 
 * echo - echo request header back to client
 */
void echo(rio_t *rp)
{
    char buf[MAXLINE];
    ...
    while (readn < MAXLINE)
    {
        size_t n = Rio_readlineb(rp, buf + readn, MAXLINE);
        readn += n;
        if (strncmp(buf + readn - n, "\r\n", 2) == 0)
        {
            break;
        }
    }
    ...
    sprintf(header, "Server: Tiny Web Server\r\n");
    Rio_writen(fd, header, strlen(header));
    sprintf(header, "Content-length: %d\r\n", readn);
    Rio_writen(fd, header, strlen(header));
    sprintf(header, "Content-type: %s\r\n\r\n", "text/plain");
    Rio_writen(fd, header, strlen(header)); //line:netp:servestatic:endserve
